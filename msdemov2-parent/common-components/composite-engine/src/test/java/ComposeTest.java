...
import com.msdemo.v2.common.composite.handler.XmlDefinitionHelper;
import com.msdemo.v2.common.composite.param.ParamMapping;
import com.msdemo.v2.common.utils.LogUtil;

@RunWith(SpringRunner.class) 
@ContextConfiguration(classes={ComposeTest.class,CompositionFactory.class})
@Configuration
public class ComposeTest{
	...
	@Bean
	public ProcessDefinition testProcess(){
		...
		ProcessDefinition pf= CompositionFactory.process(PROCESS_NAME)
...
//						.addFlow(SimpleFlow.builder().name("Error").beanName("Abnormal").method("abnormal").build())
	...
	public void printProcessFlow() throws Exception{
		logger.info("processflow json: {}",
				new ObjectMapper().writerWithDefaultPrettyPrinter()
					.writeValueAsString(CompositionFactory.get(PROCESS_NAME)));
	...
	public void beanComponse() throws Exception{
		...
		LogUtil.cost(logger, start, "load LogUtil and SpelExpressionParser class");
		
		CompositionContext context=CompositionFactory.executeChain(PROCESS_NAME,req);
...
//		});
		...
		if (context.getException()==null)
			...
			LogUtil.exceptionLog(logger, context.getException());
	}
	...
	public void fromXml() throws Exception{
		String xml=CompositionFactory.get(PROCESS_NAME).toXml();
		logger.info(xml);
		ProcessDefinition pf=XmlDefinitionHelper.fromXml(xml);
		logger.info(pf.toXml());
	}
	
	@Test
	public void toXml(){
		logger.info(CompositionFactory.get(PROCESS_NAME).toXml());
	}
}
